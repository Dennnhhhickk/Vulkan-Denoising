...
const int WIDTH          = 100;  // Size of rendered mandelbrot set.
const int HEIGHT         = 100;  // Size of renderered mandelbrot set.
const int R = 3;    // Workgroup size in compute shader.
...
public:
    ...
    void run()
    {
      ...
      */
      ...
      size_t bufferSize = sqr(2 * R + 1) * sizeof(Pixel) + 3 * sizeof(Pixel) * WIDTH * HEIGHT;
...
//____________________________________________________________________________________________________________________________
  ...
  std::cout << "makeing image ..." << std::endl;
  std::vector<std::vector<Pixel> > tmp = IMAGE();
  std::cout << "write to device memory ..." << std::endl;
  writeImageToDeviceMemory(device, bufferMemory, sqr(2 * R + 1) * sizeof(Pixel), WIDTH, HEIGHT, tmp);
  std::cout << "start processing ..." << std::endl;
  for (int ry = R; ry < WIDTH - R; ++ry) {
    for (int rx = R; rx < HEIGHT - R; ++rx) {
      std::vector<std::vector<Pixel> > tmp1;
      tmp1.resize(2 * R + 1);
      for (int i = -R; i <= R; ++i) {
        for (int j = -R; j <= R; ++j) {
          // tmp[i + ry][j + rx].r = 0.0f;
          // tmp[i + ry][j + rx].g = 1.0f;
          tmp1[i + R].push_back(tmp[i + ry][j + rx]);
        }
      }
      writeImageToDeviceMemory(device, bufferMemory, 0, 2 * R + 1, 2 * R + 1, tmp1);


      std::cout << "compiling shaders " << rx << ' ' << ry << " ... " << std::endl;
      ...
      tmp[ry][rx] = PIXEL(device, bufferMemory, sqr(2 * R + 1) * sizeof(Pixel), WIDTH, HEIGHT);
...
//____________________________________________________________________________________________________________________________
      ...
      saveRenderedImageFromDeviceMemory(device, bufferMemory, sqr(2 * R + 1) * sizeof(Pixel), WIDTH, HEIGHT, "1.bmp");
      saveRenderedImage(tmp, WIDTH, HEIGHT, "2.bmp");
...
//____________________________________________________________________________________________________________________________
  ...
  std::vector<std::vector<Pixel> > IMAGE() {
    ...
    static Pixel PIXEL(VkDevice a_device, VkDeviceMemory a_bufferMemory, size_t a_offset, int a_width, int a_height)
    {
      const int a_bufferSize = a_width * sizeof(Pixel);

      void* mappedMemory = nullptr;
      void* mappedMemory1 = nullptr;
      ...
      for (int i = 0; i < a_height; i += 1) 
      {
        ...
        vkMapMemory(a_device, a_bufferMemory, offset + HEIGHT * WIDTH * sizeof(Pixel), a_bufferSize, 0, &mappedMemory);
        Pixel* pmappedMemory = (Pixel *)mappedMemory;
        std::vector<float> tt;
        tt.resize(a_width);
        for (int j = 0; j < a_width; j += 1) {
          tt[j] = pmappedMemory[j].r;
        }
        vkUnmapMemory(a_device, a_bufferMemory);
        // Get the color data from the buffer, and cast it to bytes.
        vkMapMemory(a_device, a_bufferMemory, offset, a_bufferSize, 0, &mappedMemory);
        pmappedMemory = (Pixel *)mappedMemory;

        for (int j = 0; j < a_width; j += 1)
        {
          res += tt[j];
          image.r += pmappedMemory[j].r * tt[j];
          image.g += pmappedMemory[j].g * tt[j];
          image.b += pmappedMemory[j].b * tt[j];
          image.a += pmappedMemory[j].a * tt[j];
        }
    ...
    static void writeImageToDeviceMemory(VkDevice a_device, VkDeviceMemory a_bufferMemory, size_t a_offset, int a_width, int a_height, std::vector<std::vector<Pixel> > ex)
    {
      ...
      // We save the data to a vector.
      // std::vector<> image = READ_IMAGE(a_width, a_height);
      ...
      for (int i = 0; i < a_height; i += 1) 
      {
        ...
        // Done reading, so unmap.
        vkUnmapMemory(a_device, a_bufferMemory);
        //vkFlushMappedMemoryRanges(a_device, a_bufferSize, 0);
      }
    }
